...
const getFontSize = (icon, size) => {
  ...
  if (['check', 'eye', 'eyeOff', 'trash', 'card', 'qr', 'home', 'bicycle', 'package', 'pin'].includes(icon)) {
    return Math.max(Math.round(size * 0.55), 10);
  }

  if (['paypal', 'apple', 'google', 'cash', 'wallet', 'map', 'route', 'call', 'message'].includes(icon)) {
...
const getMinWidth = (icon, size) => {
  if (['eye', 'eyeOff', 'card', 'trash', 'check', 'qr', 'home', 'bicycle', 'package', 'pin', 'map', 'route', 'call', 'message'].includes(icon)) {
